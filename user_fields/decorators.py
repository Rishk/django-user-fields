        ...
        if 'extra_fields' in kwargs:
            ...
            for extra_field in self.extra_fields:
                ...
                if field_type == 'char':
                    ...
                    self.fields[extra_field.name] = forms.CharField(max_length=extra_field.max_length)

                elif field_type == 'text':

                    self.fields[extra_field.name] = forms.CharField(widget=forms.Textarea)
                    ...
                    self.fields[extra_field.name] = forms.ChoiceField(choices=choices_tuple)
                ...
                self.fields[extra_field.name].initial = self.instance.retrieve_extra_data(extra_field)

        else:

            raise ValueError('Extra fields could not be found. Please ensure you pass extra fields to the form '
